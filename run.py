...
class Animation:
    ...
    def setPixel(self, i, x, y, r, g, b):
        if(not(x >= 0 and x < self.w and y >= 0 and y < self.h)):
            print('Unable to set a pixel, x: ' + str(x) + ', y: ' +
                  str(y) + ' location out of picture frame')
            return False
...
class Display:
                    ...
                    while(True):
                        if self.ser.in_waiting:
                            ...
                            if(num == self.number):
                                # if(num == 0):
                                print(
                                    'This is my Arduino! Setting the port to ' + port)
                                self.port = port
    ...
    def sendFrame(self, frame):
        if self.connected() and self.isReady():
            ...
            j = 0
            while (i < len(frame) + 1):
                data[i + 0] = int(frame[DISPLAY_PIXEL_ORDER[j] * 3]
                                  * self.whiteBalance[0] * self.brightness)
                data[i + 1] = int(frame[DISPLAY_PIXEL_ORDER[j] * 3 + 1]
                                  * self.whiteBalance[1] * self.brightness)
                data[i + 2] = int(frame[DISPLAY_PIXEL_ORDER[j] * 3 + 2]
                                  * self.whiteBalance[2] * self.brightness)
                j += 1
                i += 3
...
class GifRenderer:
        ...
        while(gif):
            ...
            for y in range(0, height):
                for x in range(0, width):
                    ...
                    if(d < self.n):
                        self.display[d].animations[-1].setPixel(
                            f, _x, _y, r, g, b)
    ...
    # very basic non well thought tru algorithm
    def play(self):
        for a in range(0, len(self.display[0].animations)):
            ...
            for f in range(0, frame_count):
                ...
                for d in range(len(self.display)):
                    self.display[d].sendFrame(
                        self.display[d].animations[a].frames[f])
...
while(run):
    ...
    try:
        # if(True):
        os.system("sudo mount -t ntfs-3g /dev/sda1 /mnt/usb")

        renderer = GifRenderer(DISPLAYS)
        renderer.connect()

        print("Clearing the screens...")
        ...
        while True:
            ...
            while(False or (now.hour >= 17 and now.hour <= 23) or (now.hour >= 0 and now.hour <= 1) or (now.hour >= 4 and now.hour <= 6)):
                renderer.play()
                now = datetime.now()
